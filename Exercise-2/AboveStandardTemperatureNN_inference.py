...
WEIGHTS_PATH = "/nn_weights/AboveStandardTemperatureNN_Model_Parameters.pt"
PREPROCESS_VALUES_PATH = "/nn_weights/AboveStandardTemperatureNN_training_preprocess_values.pickle"

# *==== Class Definitions ====*


class AboveStandardTemperatureNN(torch.nn.Module):
    """
...
def load_values():
    ...
    model = AboveStandardTemperatureNN(input=7, output=1)
...
if (__name__ == "__main__"):
    argp = argparse.ArgumentParser(
        description="Predict time of weilding process that is over 723 Celcius temperature, given desired inputs")
    ...
    # 0.005,200,1200,0.0035,0.025,0.025,0.0025,
    # 12.08
    ...
    dt = model.predict(input_scaled)
    print(
        f"Time over 723 Celcius degrees for given input is {dt[0][0]:.2f}")
